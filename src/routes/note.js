...
router.post('/', (req, res) => {
  ...
  */
  ...
  var sql = 'INSERT INTO Asses3(text,dateCreated,lastModified) VALUES ("'+newText+ '","'+dateTime+'","'+dateTime+'")';
   connection.query(sql, function (err, results) {
      if (err){ throw err;}
      console.log("row inserted");
      var sql = "select * from Asses3 where id  = '"+results.insertId+"'";
...
router.put('/', (req, res) => {
  ...
  var sql = "UPDATE Asses3 SET text = '" +newText+"',"+"lastModified = '"+d+"'"+"WHERE id  = '"+noteId+"'";
  connection.query(sql, function (err,updatedNote) 
  {
     if (err){ throw err;}
     console.log("row inserted");
     var sql = "select * from Asses3 where id  = '"+noteId+"'";
     connection.query(sql, function (err, updatedNote ) 
     {
      if (err)
      { 
...
router.delete('/', (req, res) => {
  ...
  const noteId = req.body.id
  var sql = "DELETE FROM Asses3 WHERE id  = '"+noteId+"'";
  connection.query(sql, function (err,updatedNote) {
    res.send()
  });
  /*
